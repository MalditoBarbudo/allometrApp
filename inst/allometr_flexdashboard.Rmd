...
```{r save_table}
downloadLink('save_csv', 'Download as a csv file')
br()
downloadLink('save_xlsx', 'Download as a xlsx file')
```
...
user_data <- reactive({
  ...
  if (extension == '.csv') {
    ...
    res <- readxl::read_excel(input$user_data$datapath) %>% dplyr::select_if(is.numeric)
  }
