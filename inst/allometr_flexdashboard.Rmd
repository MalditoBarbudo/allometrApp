...
user_data <- reactive({
  ...
  if (extension == '.csv') {
    readr::read_csv(input$user_data$datapath)
  } else {
    readxl::read_excel(input$user_data$filepath)
  }
})
```
...
renderTable({
  ...
  user_data() %>%
    allom_calculate(
      ...
      name = allom_description(id = input$allometry_selector)$variable_dependent
    )
})
```
